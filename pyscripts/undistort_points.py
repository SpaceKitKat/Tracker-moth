...
def undistort_points(points2,cam_matrix,distortion_profile):
  ...
  points3=np.array([points2])#np.array([[n] for n in points2])
  undistPoints=cv2.undistortPoints(points3, cam_matrix,distortion_profile,None,cam_matrix)
  return undistPoints[0]
...
def plot_data(hdata,mdata):
  ...
  ax.scatter(mdata[:,1],mdata[:,2],s=5,c='r',marker="x",label="machine")
...
def main():
  ...
  new_cam_matrix=cv2.getOptimalNewCameraMatrix(cam_matrix,cam_distortion,(640,480),1)
